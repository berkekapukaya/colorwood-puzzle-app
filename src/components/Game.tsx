...
import { Container, Box } from '../types';
...
const MAX_BOXES_PER_CONTAINER = 5;
const INITIAL_EMPTY_CONTAINERS = 1;

const Game: React.FC = () => {
    const [containers, setContainers] = useState<Container[]>(() => {
        const createFixedBoxes = (): Box[] => {
            return COLORS.flatMap(color =>
                Array.from({ length: BOXES_PER_COLOR }, () => ({ color }))
            ).sort(() => Math.random() - 0.5); // Shuffle the boxes
        };

        const allBoxes = createFixedBoxes();
        const randomizeBoxes = (boxes: Box[]): Container[] => {
            const shuffled = [...boxes].sort(() => Math.random() - 0.5);
            const totalContainers = 3 + INITIAL_EMPTY_CONTAINERS; // 3 initial containers + empty ones
            ...
            shuffled.forEach((box) => {
                const availableContainers = containers.slice(0, 3).filter(c => c.boxes.length < MAX_BOXES_PER_CONTAINER);
